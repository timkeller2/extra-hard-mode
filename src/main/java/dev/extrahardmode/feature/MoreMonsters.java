...
public final class MoreMonsters implements FeatureModule {
    ...
    private static final AtomicBoolean PACK_COUNT_MIXIN_APPLIED = new AtomicBoolean();
    private static final AtomicBoolean PACK_COUNT_WARNED = new AtomicBoolean();
    ...
    public void bootstrap(FeatureBus bus) {
        ServerLifecycleEvents.SERVER_STARTED.register(server -> server.execute(MoreMonsters::warnIfPackMixinMissing));
    }

    public static void markPackCountMixinApplied() {
        PACK_COUNT_MIXIN_APPLIED.set(true);
    }

    public static void warnIfPackMixinMissing() {
        if (PACK_COUNT_MIXIN_APPLIED.get()) {
            return;
        }
        if (PACK_COUNT_WARNED.compareAndSet(false, true)) {
            ExtraHardModeMod.LOGGER.warn(
                    "EHM NaturalSpawner pack-size inject did not apply; more-monsters pack xN disabled. Lithium or another mixin may have replaced spawnCategoryForPosition.");
        }
    }
